...
export default function RegisterPage() {
  ...
  const validate = (): string => {
    ...
    if (name.trim().length > 80) return 'Nome deve ter no máximo 80 caracteres.';
    if (password.length < 8) return 'Senha deve ter no mínimo 8 caracteres.';
    if (password.length > 12) return 'Senha deve ter no máximo 12 caracteres.';
    return '';
  };
  ...
  // Feedback visual inline para a senha
          ...
          <div className="p-8 space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="relative">
                <User className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#141414]/30" />
                <input
                  type="text"
              ...
              <div className="space-y-1.5">
                <div className="relative">
                  ...
                  <input
                    type="password"
                    placeholder="Senha (8 a 12 caracteres)"
                    value={password}
                ...
                {passwordStrength && (
                  <p className={`text-xs ml-1 ${
                    ...
                    {passwordStrength === 'fraca' && `Muito curta — faltam ${8 - password.length} caractere(s).`}
                    {passwordStrength === 'ok' && `✓ Tamanho válido (${password.length}/12)`}
                    {passwordStrength === 'longa' && `Muito longa — máximo 12 caracteres.`}
                  </p>
                )}
              </div>
            ...
            <p className="text-center text-sm text-[#141414]/40">
              Já tem uma conta?{' '}
              <Link to="/auth" className="text-[#5A5A40] font-bold hover:underline">
                Entrar
              </Link>
            </p>
          </div>
